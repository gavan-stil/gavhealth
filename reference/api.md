...
| `/api/strength/sessions?days=N` | Per-session aggregates: id, **date** (non-null after T15-2b fix), session_date, activity_log_id, duration_mins, avg_hr, calories, total_sets, total_reps, total_load_kg, avg_load_per_set_kg, exercises[], **category** (push/pull/legs/abs/mixed). **category is session_label-first (fixed 2026-07-08)** — the user's explicit split wins; muscle-group inference only for unlabelled sessions. | — |
