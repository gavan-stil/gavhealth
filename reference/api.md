...
| `/api/strength/sessions?days=N` | Per-session aggregates: id, **date** (non-null after T15-2b fix), session_date, activity_log_id, duration_mins, avg_hr, calories, total_sets, total_reps, total_load_kg, avg_load_per_set_kg, **link_confirmed** (true = user linked manually; false = auto-linked by save-match/sweep or unlinked), exercises[], **category** (push/pull/legs/abs/mixed). **category is session_label-first (fixed 2026-07-08)** — the user's explicit split wins; muscle-group inference only for unlabelled sessions. | — |
| `POST /api/strength/sessions/backfill-links` | Idempotent sweep: links unlinked sessions to unclaimed same-date `workout` activity rows (only when exactly 1↔1 on that date), copies split label to `activity_logs.workout_split` when NULL. Also runs on every builder save. Returns `{ linked: N, pairs: [...] }`. Added 2026-07-08, see `archive/specs/volume-bw-bugs.md`. | — |
...
| `/api/strength/exercise/:id/history?days=N` | Per-exercise session history (chronological): `[{ session_date, sets: int, total_reps, max_reps_in_set, top_weight_kg, top_effective_kg, session_volume_kg, estimated_1rm }]`. `sets` is a count integer. `top_weight_kg` = external/added weight only (0 for pure BW — PB-flag semantics, do not change). `top_effective_kg` = BW-inclusive max per-set load (added 2026-07-08). **`session_volume_kg` and `estimated_1rm` are BW-inclusive** — formula: `reps × (bodyweight_at_session + weight_kg)`; 1RM = Epley on effective load (fixed 2026-07-08, see `archive/specs/volume-bw-bugs.md`). Use last element for "previous session". | — |
| `GET /api/strength/sessions/:id/detail` | **T24 (curl-verified 2026-07-10).** One session's exercises in COMPLETION ORDER (from `strength_sets.set_number`). `{ session_id, date, start_time, session_label, total_sets, total_reps, total_volume_kg, exercises: [{ name, sets, reps, avg_reps, volume_kg (BW-inclusive), top: { type: "kg"\|"bw"\|"bw+", kg }, set_details: [{ load_type, kg (added/external, null for bw), reps, volume_kg }] }] }`. Used by DayDetailSheet strength card. 404 if unknown id. | — |
...
| `GET /api/log/strength/recent/{split}?limit=5` | `RecentSession[]` — up to `limit` (max 10) sessions, sorted: PBs first → most_loaded → chrono desc. **Session `is_pb` = this session's BW-inclusive volume strictly beat every earlier session of the split (a volume record when it happened); per-exercise `is_pb` dots = strictly beat an earlier session's best weight/reps (fixed 2026-07-08)** — first appearances and ties don't count. Response: `{ id, date, start_time, exercise_count, total_sets, avg_reps_per_set, total_volume_kg, is_pb, most_loaded, exercises: RecentSessionExercise[], raw_exercises: WorkoutExercise[] }`. `raw_exercises` = original JSONB for `onLoad`. Volume is BW-inclusive (bw/bw+ reps × bodyweight lookup + added kg — matches `/api/strength/sessions`, fixed 2026-07-08). 400 if split invalid. |
