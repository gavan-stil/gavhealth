Below are the details of the data you've exported from Health Mate:

activities.csv: Activities history

aggregates_calories_earned.csv: Active calories burned regrouped by days

aggregates_calories_passive.csv: Passive calories burned regrouped by days

aggregates_distance.csv: Travelled distance regrouped by days

aggregates_elevation.csv: Floor climbed regrouped by days
· Value : (steps number)

aggregates_manual_spo2.csv: Manual SpO2 data grouped by days
· Sum : 

aggregates_steps.csv: Steps data regrouped by days
· Value : (steps number)

bp.csv: Blood Pressure Data
· Heart Rate : (bpm)
· Systolic : (mmHg)
· Diastolic : (mmHg)

height.csv: Height Measurements
· Height : (meters)

manual_spo2.csv: Manual SpO2 (Oxygen Satutation) Data
· Blood oxygen level : (%)

note.csv: User note for body temperature measurements

raw_bed_calories-earned.csv: Sleep monitor calories earned data

raw_bed_hr.csv: Sleep monitor heart rate data
· Duration : (seconds)
· Value : (bpm)

raw_bed_maximum_movement.csv: Sleep monitor maximum movement data

raw_bed_movement_pim.csv: Sleep monitor movement pim data

raw_bed_respiratory-rate.csv: Sleep monitor respiratory rates
· Duration : (seconds)

raw_bed_sleep-state.csv: Sleep monitor sleep data
· Duration : (seconds)
· Value : ( 0 -> awake; 1 -> light sleep; 2 -> deep sleep; 3 -> rem)

raw_bed_snoring.csv: Sleep monitor snoring data
· Duration : (seconds)
· Value : (0 -> not snoring; 1 -> snoring)

raw_hr_hr.csv: Heart rate data
· Heart Rate : (bpm)

raw_location_altitude.csv: Altitude data

raw_location_direction.csv: Direction data

raw_location_gps-speed.csv: GPS speed data

raw_location_horizontal-radius.csv: Horizontal radius data

raw_location_latitude.csv: Latitude data
· Duration: (seconds)

raw_location_longitude.csv: Longitude data
· Duration: (seconds)

raw_location_vertical-radius.csv: Vertical radius data

raw_spo2_auto_spo2.csv: Automatic SpO2 data
· Duration : (seconds)
· Value : (%)

raw_spo2_quality_score.csv: Quality score of SpO2 data

raw_swim_lap-pool.csv: Pool lap data
· Duration: (minutes)

raw_tracker_calories-earned.csv: Calories earned data
· Duration : (seconds)
· Value : (number of calories)

raw_tracker_distance.csv: Distance data
· Duration : (seconds)
· Value : (meters)

raw_tracker_elevation.csv: Elevation data
· Duration : (seconds)
· Value : (meters)

raw_tracker_lap-pool.csv: Lap pool data
· Duration : (seconds)
· Value : (number of laps)

raw_tracker_sleep-state.csv: Sleep trackers data
· Duration : (seconds)
· Value : (0 -> awake; 1 -> light sleep; 2 -> deep sleep)

raw_tracker_steps.csv: Steps data
· Duration : (seconds)
· Value : (steps number)

signal.csv: Your ECG signals
· Frequency: Sampling frequency in Hz
· Duration: Duration in seconds
· Wear Position: Position of the device when tracking measurement :
 - 0 -> right wrist;  1 -> left wrist
 - 2 -> right arm;  3 -> left arm
· Signal: Full data set of the ECG recordings
 - Value in microvolts (μV) for ECG recorded with a watch.
 - Value in millivolts (mV) for ECG recorded with a blood pressure monitor.

sleep.csv: Sleep data

weight.csv: Weights Measurements
· Weight : Weight (kg)
· Fat mass : Fat mass (kg)
· Bone mass : Bone mass (kg)
· Muscle mass : Muscle mass (kg)
· Hydration : Hydration (kg)

