...
- **T15 — Trends Redesign: Energy Balance + Strength Quality** — Spec: `tasks/T15-trends-energy-balance.md`
  - T15-1: Energy Balance Chart (new endpoint + React component replacing CorrelationSummary)
  - T15-2: Strength Quality Scatter (fix date bug + new scatter component)
  - T15-3: Run HR Zones — BLOCKED until backend populates `activity_logs.hr_zone_*`
...
- ~~**T24 — Calendar strength card parity with session logging**~~ ✅ SHIPPED 2026-07-10 — DayDetailSheet strength card reads like the builder: exercises in completion order (currently alphabetical from `ARRAY_AGG(DISTINCT)`), table columns EXERCISE / SETS / REPS / AVG (reps per set) / VOL (BW-inclusive) / TOP (`20kg` \| `BW` \| `BW+20` — currently blank for pure BW), plus per-exercise dropdown revealing the actual set breakdown. Mockup: `archive/session-card-mockup.html` (approved). Data source: `manual_strength_logs.exercises` JSONB via `bridged_session_id` (preserves order + per-set load types); built as `GET /api/strength/sessions/{id}/detail` (from `strength_sets.set_number` — avoids the stale-kg JSONB trap). TOP semantics per `archive/specs/volume-bw-bugs.md`.
